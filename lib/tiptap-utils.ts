...
export const handleImageUpload = async (
  ...
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_BLOG_API_URL}/media`, {
      method: "POST",
      body: formData,
      signal: abortSignal,
      credentials: 'include',
    ...
    return `${process.env.NEXT_PUBLIC_BLOG_API_URL}${data.url}`;
  } catch (error: any) {
    if (error.name === 'AbortError') throw new Error("Upload cancelled");
    throw error;
  }
};
